...
10. = 과세표준
11. × 세율 적용 + 누진공제 = 산출세액
12. − 근로소득 세액공제 (tiered + 총급여별 한도 74/66/50/20만)
13. − 자녀 세액공제 (만 8세 이상만, 연도별 금액)
14. − 연금계좌 세액공제 (총급여 5500만 기준 15%/12%)
15. − 의료비 세액공제 (총급여 3% 초과분 / 일반 700만 cap·난임 30%·미숙아 20%·본인 등 한도없음)
16. − 교육비 세액공제 (15%, 항목 카테고리별 한도)
17. − 기부금 세액공제 (일반 1천만 이하 15%/초과 30% · 고향사랑/정치 10만↓ 100/110)
...
> ⚠️ **표준세액공제 자동 선택**: 매 계산 시 ① 특별공제(특별소득공제 4대보험 중 건강·고용·장기요양 + 의료/교육/일반기부/보험/월세 세액공제) 적용분과 ② 표준세액공제 13만원 적용분 **두 시나리오의 결정세액을 비교해 작은 쪽을 채택** (`evalRoute`). 고향사랑·정치자금 기부는 표준 선택과 무관하게 항상 적용. 저소득(특별공제 효과 < 13만) 사용자는 자동으로 표준세액공제 적용됨.
...
### 정확도 개선 완료 (2026-06 반영)
직접 영향이 큰 9건의 정확도 이슈를 수정함:
- ✅ **근로소득세액공제 한도 차등** — 총급여별 74/66/50/20만 (`limitBrackets`)
- ✅ **자녀세액공제 만 8세 이상** 조건 + **연도별 금액** (2024 15/35/+30만, 2025~ 25/55/+40만)
- ✅ **의료비 700만 cap + 차등 공제율** (난임 30%/미숙아 20%/본인·65+·장애인 한도없음/안경 50만) — `medicalItems` 기반
- ✅ **교육비 카테고리별 한도** (미취학·초중고 300만, 대학 900만) — `educationItems` 기반
- ✅ **고향사랑기부 100/110** (10만 이하 지방세 합산 100%) + 연 한도(2024 500만/2025~ 2천만)
- ✅ **정치자금기부** 반영 (10만↓ 100/110, ~3천만 15%, 초과 25%)
- ✅ **표준세액공제 13만원** 2-시나리오 자동 선택 (`evalRoute`)
- ✅ **부녀자공제 50만** (`isFemaleWorker` 토글, 종합소득 3천만↓, 한부모와 중복 불가)

### 남은 한계 (단순화·누락)
- **교육비 1인 한도 근사** — 항목이 인별(person) 구분을 갖지 않아 카테고리 항목 단위로 한도 적용
- **의료비 3% 기준금액 차감 순서** — 낮은 공제율(일반)부터 차감하는 단순화 모델
- **근로소득공제 2,000만 cap 누락** — 10억+ 소득자 영향 (일반 사용자 무관)
- **월별 간이세액(`estimateMonthlyIncomeTax`)이 부양가족 수 미반영** — 기납부 추정 근사 (직접 입력으로 보완 가능)
- **국민연금 기준소득월액 상한 590만** — 실제 617만(2024.7~)과 소폭 차이
- **인적공제 부양가족 나이/소득 요건 미강제** — `hasIncome` 토글로만 판정
- 부양가족 출산·입양 추가공제, 장애인전용보험 별도 한도 등 미반영
...
- **교육비 인별 한도 정밀화** — 현재는 항목 단위 근사, 부양가족 1인 단위로 정밀 적용
